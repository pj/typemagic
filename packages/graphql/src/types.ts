...
export type GetTypeScalar<Scalar> =
  [GetUnderlyingType<Scalar>] extends [infer Item]
    ? Item extends GraphQLScalarType
      ? Item extends {name: "DateTime"}
        ? Date
        : Item extends {name: "Boolean"}
          ? boolean
          : Item extends {name: "String"}
            ? string
            : Item extends {name: 'Int'}
              ? number
              : Item extends {name: 'Float'}
                ? number
                : "Unknown GraphQL Scalar"
      : "Not a GraphQL Scalar"
    : "Should not happen"

export type IsSchemaScalar<Item> =
  [GetUnderlyingType<Item>] extends [infer Type]
    ? GetTypeScalar<Type> extends "Unknown GraphQL Scalar"
      ? false
      : true
    : "Should not happen"
...
export type TransformResolverToType<Schema> =
  Schema extends {type: infer CompileTimeType, nullable?: infer Nullable, array?: infer IsArray}
    ? CreateTypeFromSchemaOptions<
        (
          IsSchemaScalar<CompileTimeType> extends true
            ...
            : CompileTimeType extends {fields: infer Fields}
              ...
              : CompileTimeType extends {enum: infer Enum}
                ...
                : CompileTimeType extends {union: infer Union}
                  ...
                  : "Unable to infer object/union/enum type"
          ), 
          Nullable, 
          IsArray
        >
    : IsSchemaScalar<Schema> extends true
      ? GetTypeScalar<Schema>
      : ["Unable to infer default scalar type", GetTypeScalar<Schema>]
